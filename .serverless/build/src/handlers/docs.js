...
var swaggerDistPath = import_path.default.dirname(
  require.resolve("swagger-ui-dist/package.json")
);
var openApiPath = import_path.default.join(__dirname, "../../openapi.json");
var STAGE = process.env.STAGE || "dev";
var ROOT = `/${STAGE}/`;
function serveFile(abs) {
  const buf = import_fs.default.readFileSync(abs);
  const type = (import_mime_types.default.lookup(abs) || "application/octet-stream").toString();
...
var handler = async (event) => {
  ...
  if (reqPath === "/docs" || reqPath === "/docs/") {
    const index = import_path.default.join(swaggerDistPath, "index.html");
    let html = import_fs.default.readFileSync(index, "utf8");
    const initScript = `
<script>
window.onload = function() {
  ...
  if (reqPath.startsWith("/docs/")) {
    ...
    const file = import_path.default.join(swaggerDistPath, rel);
    if (import_fs.default.existsSync(file)) return serveFile(file);
  }
