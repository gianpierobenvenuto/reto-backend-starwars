...
var almacenar_exports = {};
__export(almacenar_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(almacenar_exports);
  ...
  try {
    if (!event.body) {
      return {
        statusCode: 400,
        body: JSON.stringify({
          error: "No se proporcion\xF3 cuerpo en la petici\xF3n"
        })
      };
    }
    ...
    const weather = coords ? await getWeatherByLatLon(coords.lat, coords.lon) : await getWeatherByLatLon("0", "0");
    const itemToStore = {
      ...parsed,
      weather,
      timestamp: Date.now(),
    ...
    const command = new import_client_dynamodb.PutItemCommand({
      ...
      Item: (0, import_util_dynamodb.marshall)(itemToStore)
    });
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=almacenar.js.map
