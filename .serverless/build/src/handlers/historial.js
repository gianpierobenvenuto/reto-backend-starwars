...
// src/handlers/historial.ts
var historial_exports = {};
__export(historial_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(historial_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    const lastKey = query.lastKey ? JSON.parse(decodeURIComponent(query.lastKey)) : void 0;
    const params = {
    ...
    const command = new import_client_dynamodb.ScanCommand(params);
    const response = await client.send(command);
    const items = response.Items ? response.Items.map((item) => (0, import_util_dynamodb.unmarshall)(item)) : [];
