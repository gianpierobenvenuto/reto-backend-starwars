...
async function getCachedFusionado(planet) {
  ...
  if (cacheAge > 30 * 60 * 1e3) return null;
  return item;
}
...
var querySchema = z.object({
  planet: z.string().min(1, 'El par\xE1metro "planet" es obligatorio')
});
async function getCoordinates(placeName) {
  try {
    ...
    const response = await axios_default.get(url2, {
      ...
      // Nominatim recomienda usar User-Agent
    });
...
var handler = async (event) => {
  try {
    ...
    if (coords) {
      ...
      weatherData = await getWeatherByLatLon("0", "0");
    }
    const fusionado = {
